...
int main(int argc, char *argv[])
{
	...
	if (send(clntSockfd, "Connected!!!\n", 
        ...
        while(1) {
            ...
            getsockopt(clntSockfd, SOL_SOCKET,SO_SNDBUF,&senderBuffSize, &optlen); /* check sender buffer size */
            recvBuff = malloc(senderBuffSize * sizeof (char));
            if ((numBytes = recv(clntSockfd, recvBuff, senderBuffSize, 0)) == -1) {
                perror("recv failed");
                exit(1);
            }
	    //I added a check to see if the size of the client's input is larger than the buffer. If that case is true an error is thrown and the program exits.
	    if(numBytes >=MAX_DATA_SIZE*2){
		perror("size fail");
		exit(1);
	    }
            recvBuff[numBytes] = '\0';
