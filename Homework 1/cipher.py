...
def vigen(key, text): #converts the plain to cipher text
    cyp_str = ""
    key_len = len(key)
    gen_list = []
...
def rotate(a_string, amount=13): #rotates the letter to change the value
...
text = raw_input("Enter message: ") #user input message
key = raw_input("Enter key: ")  #user input key
